...
PaymentManager.prototype.createCustomer = function (token, user) {
    ...
    var stripeObject;
    console.log("** USER SIGNUP **: Payment.createCustomer(): stripeToken = " + token + ", plan=" + user.billing.options);
    ...
    stripe.customers.create( stripeObject, function (err, success) {
        if (!!err) {
            console.log("** USER SIGNUP **: ERROR: Payment.createCustomer(): " + err);
            deferred.reject(err);
        } else {
            console.log("** USER SIGNUP **: Payment.createCustomer(): " + success);
