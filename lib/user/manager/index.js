...
UserManager.prototype.find = function(options) {
    logger.info("### Find user: " + JSON.stringify(options));
    if (!options) {
        options = {
            criteria: {}
        };
...
function executeQuery(query) {
    ...
    logger.info('### Evergram-common: Start executing query');
    query.exec(function(err, results) {
        if (err) {
            logger.error('Evergram-common: Error executing query - ' + err);
            deferred.reject(err);
        } else {
            logger.info('### Evergram-common: Finish executing query - ' + JSON.stringify(results));
